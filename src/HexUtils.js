...
class HexUtils {
  ...
  static lengths(hex) {
    ...
    ///ToDo RADIX?  used 10 for no reason at all
    return parseInt((Math.abs(hex.q) + Math.abs(hex.r) + Math.abs(hex.s)) / 2, 10);
  }
  ...
  static hexToPixel(hex, layout, size) {
    const s = 1;
    const M = new Orientation(Math.sqrt(3.0), Math.sqrt(3.0) / 2.0, 0.0, 3.0 / 2.0, Math.sqrt(3.0) / 3.0, -1.0 / 3.0, 0.0, 2.0 / 3.0, 0.5);
      // const s = layout.spacing;
      // const M = layout.orientation;
    // let x = (M.f0 * hex.q + M.f1 * hex.r) * layout.size.x;
    // let y = (M.f2 * hex.q + M.f3 * hex.r) * layout.size.y;
  let x = (M.f0 * hex.q + M.f1 * hex.r) * size;
  let y = (M.f2 * hex.q + M.f3 * hex.r) * size;
    // Apply spacing
    x = x * s;
    y = y * s;
    x = x + ((Math.sqrt(3.0) / 2) * size) + 1;
    y = y + 1 + (size * 1.0);
    // return new Point(x + layout.origin.x, y + layout.origin.y);
    // console.error('returning hexToPixel point')
    // console.error(x, y);
